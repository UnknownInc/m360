...
import { Form, Header, Segment, Table, Icon, Checkbox, Button, Divider, Message, Popup, Responsive,Modal, FormButton } from 'semantic-ui-react';
...
class AdminQuestionsPage extends Component {
  ...
  componentDidMount() {
    ...
    fetch(`${QUESTIONS_API}/api/questions`, { headers})
      .then(res => res.json())
      .then(questions => {
  ...
  handleSaveChanges = async ()=> {
    ...
    if (newQuestions.length>0) {
      ...
      try {
        const res = await fetch(`${QUESTIONS_API}/api/questions`, { headers, method: 'POST',  body})
    ...
    if (modifiedQuestions.length>0) {
      try {
        ...
        const res = await fetch(`${QUESTIONS_API}/api/questions`, { headers, method: 'PUT',  body})
        if (res.ok) {
